...
def analyze_market_sentiment(oi_pcr: float, chgoi_pcr: float, volume_pcr: float) -> dict:
    """Analyze overall market sentiment based on all three PCR types"""
    
    # Classify each PCR
    oi_bullish = oi_pcr < 0.8
    oi_bearish = oi_pcr > 1.2
    oi_neutral = 0.8 <= oi_pcr <= 1.2
    
    # REVERSED LOGIC FOR ChgOI: Lower PCR = Bearish (call unwinding/put building)
    chgoi_bullish = chgoi_pcr > 1.0  # Higher ChgOI PCR = more put unwinding = bullish
    chgoi_bearish = chgoi_pcr < 1.0  # Lower ChgOI PCR = more call unwinding = bearish
    chgoi_strong_bullish = chgoi_pcr > 1.5  # Very high = strong put unwinding
    chgoi_strong_bearish = chgoi_pcr < 0.5  # Very low = strong call unwinding
    
    volume_bullish = volume_pcr < 0.7
    volume_bearish = volume_pcr > 1.3
    volume_neutral = 0.7 <= volume_pcr <= 1.3
    ...
    if volume_bullish and chgoi_bullish and (oi_neutral or oi_bullish):
        sentiment = "STRONG BULLISH"
        confidence = "HIGH"
        description = "Fresh bullish activity with strong volume and new call positions being built"
        action_suggestion = "Consider bullish strategies - calls or bull spreads"
        
    elif volume_bullish and chgoi_strong_bullish:
        sentiment = "BULLISH"
        confidence = "HIGH" 
        description = "Strong bullish momentum with aggressive call buying"
        action_suggestion = "Monitor for continuation, consider momentum plays"
    
    # Strong Bearish Scenarios
    elif volume_bearish and chgoi_bearish and oi_bullish:
        sentiment = "BEARISH REVERSAL"
        confidence = "HIGH"
        description = "Sentiment shift detected - heavy put buying despite previously bullish OI"
        action_suggestion = "Caution advised - consider protective puts or profit booking"
        
    elif volume_bearish and chgoi_strong_bearish:
        sentiment = "STRONG BEARISH"
        confidence = "HIGH"
        description = "Heavy put accumulation with strong bearish volume"
        action_suggestion = "Consider bearish strategies - puts or bear spreads"
    
    # Consolidation/Range-bound
    elif volume_neutral and abs(chgoi_pcr - 1.0) < 0.3 and oi_neutral:
        sentiment = "CONSOLIDATION"
        confidence = "MEDIUM"
        description = "Range-bound market with balanced put-call activity"
        action_suggestion = "Consider range-bound strategies - iron condors, straddles"
    
    # Mixed Signals
    elif (volume_bullish and chgoi_bearish) or (volume_bearish and chgoi_bullish):
        sentiment = "MIXED SIGNALS"
        confidence = "LOW"
        description = "Conflicting signals between current trading and recent position building"
        action_suggestion = "Wait for clearer direction or use neutral strategies"
    
    # Moderate scenarios
    elif volume_bullish or chgoi_bullish:
        sentiment = "BULLISH BIAS"
        confidence = "MEDIUM"
        description = "Some bullish signals present but not overwhelming"
        action_suggestion = "Cautiously bullish approach with risk management"
        
    elif volume_bearish or chgoi_bearish:
        sentiment = "BEARISH BIAS" 
        confidence = "MEDIUM"
        description = "Some bearish signals present but not overwhelming"
        action_suggestion = "Cautiously bearish approach with risk management"
    
    # Default neutral
    else:
        sentiment = "NEUTRAL"
        confidence = "MEDIUM"
        description = "Balanced market conditions without clear directional bias"
        action_suggestion = "Consider neutral strategies or wait for clearer signals"
    ...
    return {
        ...
        "chgoi_signal": "Bullish" if chgoi_bullish else "Bearish" if chgoi_bearish else "Neutral", 
        "volume_signal": "Bullish" if volume_bullish else "Bearish" if volume_bearish else "Neutral"
    }
